...
\Rcode{fvarLabels(experssionSet)} contains the feature labels.
...
unique these specific genes int he signature are for this task. the function
...
The upper-right plot shows performance of the v'ant veer signature relative to 1,000
randomly chosen signature of comprised of an equal number of gene. The
upper-left plot shows the performance compared to 48 known
