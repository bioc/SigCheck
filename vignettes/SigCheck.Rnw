...
\date{Edited: May 20, 2015; Compiled: \today}
...
For the NKI dataste, the \Rcode{varLabel} named \Rcode{"series"} specifies which 
...
If the data are divded into training and validaiton samples, the samples 
that comprise the validaiton set should be specified using
